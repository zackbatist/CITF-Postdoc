(function() {
  ...
  function updateCategoryState(prefix) {
    ...
    if (!statusEl) {
      ...
      const titleDiv = category.querySelector('.category-title');
      if (titleDiv) {
  ...
  function updateAllCategoryStates() {
    document.querySelectorAll('.filter-category').forEach(cat => {
      const prefix = cat.dataset.prefix;
      if (prefix) {
  ...
  function updateAllFilters() {
    ...
    document.querySelectorAll('.coding-table').forEach(table => {
      const tbody = table.querySelector('tbody');
      const rows = Array.from(tbody.querySelectorAll('tr')).filter(row => 
        row.dataset.lineNumber // Only process actual data rows
      );
      
      // Remove existing gap rows
      tbody.querySelectorAll('.gap-row').forEach(gap => gap.remove());
      
      let previousRowHidden = false;
      let hiddenCount = 0;
      ...
      rows.forEach((row) => {
        ...
        if (!speakerMatch) {
          ...
          shouldHide = !state.showUncoded;
        } else {
          // Coded rows: show if any code is active
          const hasActiveCode = Array.from(tags).some(tag => !tag.classList.contains('inactive'));
          shouldHide = !hasActiveCode;
        }
        
        // Apply hiding
        if (shouldHide) {
          row.classList.add('hidden');
          if (!previousRowHidden) {
            hiddenCount = 1;
          } else {
            hiddenCount++;
          }
          previousRowHidden = true;
        } else {
          // Insert gap row if previous rows were hidden
          if (previousRowHidden && hiddenCount > 0) {
            const gapRow = document.createElement('tr');
            gapRow.className = 'gap-row';
            const gapCell = document.createElement('td');
            gapCell.colSpan = 4;
            gapCell.textContent = `[${hiddenCount} line${hiddenCount > 1 ? 's' : ''} hidden]`;
            gapRow.appendChild(gapCell);
            tbody.insertBefore(gapRow, row);
            hiddenCount = 0;
          }
          
          row.classList.remove('hidden');
          previousRowHidden = false;
        }
      });
      
      // Add final gap if table ends with hidden rows
      if (previousRowHidden && hiddenCount > 0) {
        const gapRow = document.createElement('tr');
        gapRow.className = 'gap-row';
        const gapCell = document.createElement('td');
  ...
  function updateStats() {
    document.querySelectorAll('.interview-section').forEach(section => {
      ...
      const allRows = Array.from(table.querySelectorAll('tbody tr')).filter(row => row.dataset.lineNumber);
      ...
      const codedRows = allRows.filter(row => row.querySelectorAll('.code-tag').length > 0);
      const uncodedRows = allRows.filter(row => row.querySelectorAll('.code-tag').length === 0);
      const visibleCoded = visibleRows.filter(row => row.querySelectorAll('.code-tag').length > 0);
      const visibleUncoded = visibleRows.filter(row => row.querySelectorAll('.code-tag').length === 0);
      
      const summary = section.querySelector('.stats-summary');
      if (summary) {
        const visibleCountEl = summary.querySelector('.visible-count');
        const visibleCodedCountEl = summary.querySelector('.visible-coded-count');
        const totalCodedCountEl = summary.querySelector('.total-coded-count');
        const visibleUncodedCountEl = summary.querySelector('.visible-uncoded-count');
        const totalUncodedCountEl = summary.querySelector('.total-uncoded-count');
        const hiddenCountEl = summary.querySelector('.hidden-count');
        const hiddenCodedCountEl = summary.querySelector('.hidden-coded-count');
        const hiddenUncodedCountEl = summary.querySelector('.hidden-uncoded-count');
        
        if (visibleCountEl) visibleCountEl.textContent = visible;
        if (visibleCodedCountEl) visibleCodedCountEl.textContent = visibleCoded.length;
        if (totalCodedCountEl) totalCodedCountEl.textContent = codedRows.length;
        if (visibleUncodedCountEl) visibleUncodedCountEl.textContent = visibleUncoded.length;
        if (totalUncodedCountEl) totalUncodedCountEl.textContent = uncodedRows.length;
        if (hiddenCountEl) hiddenCountEl.textContent = total - visible;
        if (hiddenCodedCountEl) hiddenCodedCountEl.textContent = codedRows.length - visibleCoded.length;
        if (hiddenUncodedCountEl) hiddenUncodedCountEl.textContent = uncodedRows.length - visibleUncoded.length;
      }
    });
  }
  
  function initializeFilters() {
    // Apply border colors to categories based on their codes
    ...
    if (speakerFilterBtn && speakerDropdown) {
      ...
      document.querySelectorAll('.speaker-checkbox').forEach(cb => {
        ...
        if (state.selectedSpeakers[speaker] === undefined) {
  ...
  function restoreState() {
    // Restore collapsed interview sections
    ...
    // Restore collapsed code categories
    Object.keys(state.categoryCollapsed).forEach(prefix => {
      if (state.categoryCollapsed[prefix]) {
        ...
        if (category) {
          ...
          if (codesList) codesList.classList.add('collapsed');
        }
      }
    });
  ...
  function getFilteredData() {
    ...
    document.querySelectorAll('.interview-section').forEach(section => {
      ...
      section.querySelectorAll('.coding-table tbody tr:not(.hidden):not(.gap-row)').forEach(row => {
  ...
  function exportCSV(data, filterParams) {
    ...
    csv += '#\n';
    csv += 'Interview,Line,Speaker,Text,Codes\n';
  ...
  function exportHTML(data, filterParams) {
    ...
    html += '</div>';
    html += '<table><thead><tr><th>Interview</th><th>Line</th><th>Speaker</th><th>Text</th><th>Codes</th></tr></thead><tbody>';
    
    data.forEach(row => {
      html += '<tr><td>' + row.interview + '</td><td>' + row.line + '</td><td>' + row.speaker + '</td><td>' + row.text + '</td><td>' + row.codes + '</td></tr>';
    });
  ...
  function exportPDF(data, filterParams) {
    ...
    const tableData = data.map(row => [row.interview, row.line, row.speaker, row.text, row.codes]);
    
    doc.autoTable({
      startY: yPos,
      head: [['Interview', 'Line', 'Speaker', 'Text', 'Codes']],
      body: tableData,
      styles: { fontSize: 8 },
  ...
  window.qcVizState = state;
  window.qcVizSaveState = saveState;
  ...
  (function() {
    ...
    if (_nav) qcInitNav(_nav, { apiBase: 'http://localhost:' + 8080 });
  })();
    loadState();
    initializeFilters();
