...
Options:
    ...
    --resolve-collisions Interactively merge sibling collisions produced by --strip-prefixes.
                         Reads qc/strip-prefixes-collisions.json (generated by --strip-prefixes
                         --dry-run). Presents numbered list of suffixed codes that can be merged
                         into their clean-name sibling. Enter numbers (e.g. 1,3,5) or 'all'.
                         Shows confirmation before executing. --dry-run previews without writing.
...
def op_strip_prefixes(dry_run):
    ...
    # Detect collisions — only among siblings (same parent)
    parent_map = {n["name"]: n.get("parent", "") for n in tree}
    unchanged_set = set(n for n in all_names if n not in rename_map)

    # Group by (stripped_name, parent) — only codes with same parent AND same stripped name collide
    sibling_groups = defaultdict(list)  # (stripped_name, parent) -> [original_names]
    for old, new in sorted(rename_map.items()):
        parent = parent_map.get(old, "")
        sibling_groups[(new, parent)].append(old)
    for name in sorted(unchanged_set):
        parent = parent_map.get(name, "")
        stripped = strip_prefix(name)  # unchanged codes have no prefix so stripped == name
        sibling_groups[(name, parent)].append(name)

    collisions = {}  # original_name -> final_name (with suffix)
    collision_groups = {}  # (stripped_name, parent) -> list of originals
    all_names_set = set(all_names)
    for (stripped, parent), originals in sibling_groups.items():
        if len(originals) > 1:
            collision_groups[(stripped, parent)] = originals
    ...
    if collision_groups:
        print(f"\n[strip-prefixes] {len(collision_groups)} sibling collision(s) resolved with suffixes:")
        for (stripped, parent), originals in sorted(collision_groups.items()):
            print(f"  parent: {parent or '(top level)'}, stripped name: {stripped}")
            for old in originals:
                final = rename_map.get(old, old)
                marker = " (suffixed)" if old in collisions else " (clean name)"
                print(f"    {old}  ->  {final}{marker}")
        print(f"  Use --resolve-collisions to interactively merge these.")
...
def op_resolve_collisions(dry_run):
    ...
    selection = input("\nEnter numbers to merge (e.g. 1,3,5 or 'all'): ").strip()
    if not selection:
        print("[aborted]")
        return
    ...
    if selection.lower() == "all":
        ...
        try:
            ...
            selected = [i for i in selected if 0 <= i < len(pairs)]
        except ValueError:
    ...
    for i in selected:
        pair = pairs[i]
        print(f"  {i+1}. merge '{pair['src']}' into '{pair['target']}'")
    ...
    for i in selected:
        pair = pairs[i]
        print(f"  merging '{pair['src']}' into '{pair['target']}'...", end=" ", flush=True)
        try:
            r = subprocess.run(
    ...
    if successful_map:
        ...
        new_codes = {}
        for name, doc in codes.items():
            new_name = successful_map.get(name, name)
            new_doc = dict(doc)
            if "parent" in new_doc and new_doc["parent"] in successful_map:
                new_doc["parent"] = successful_map[new_doc["parent"]]
            new_codes[new_name] = new_doc
        codebook["codes"] = new_codes
        new_tree = []
        for node in codebook.get("tree", []):
            new_node = dict(node)
            new_node["name"] = successful_map.get(node["name"], node["name"])
            if node.get("parent") in successful_map:
                new_node["parent"] = successful_map[node["parent"]]
            new_tree.append(new_node)
        codebook["tree"] = new_tree
        save_codebook(codebook)
...
def main():
    ...
    resolve_collisions  = "--resolve-collisions"  in sys.argv
